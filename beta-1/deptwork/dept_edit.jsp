...
<%/*
	...
	if(retFlag != null){
		if(retFlag.equals("true"))
			out.println("alert('修改部门成功！')");
		else
			out.println("alert('修改部门失败！')");
...
<form name="thisForm" method="post" action="<%=request.getContextPath()%>/MainController.do?ActionName=com.dfkj.eoa.actions.deptwork.EditEoaDeptAction&NoCache=1&NextPage=/deptwork/dept_transfer.jsp&actionFlag=edit">
  <table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" class="content_table">
	...
	<table id=dbcolortable width="100%" border="0" cellspacing="1" cellpadding="4" align="center">
          ...
          <tr> 
            <td>上级部门</td>
            <td><input name="parent_name" type="text" class="input_data" size="30" value="<%if(vo.getParentId() == null) out.print("部门"); else out.print(vo.getParentId());%>" readonly>&nbsp;<input type="button" class="input_button" name="deptSel" value="选择部门" onClick="selDept()"></td>
          </tr>
          <tr> 
            <td>部门类型</td>
          ...
          <tr> 
            ...
            <td><select name="enabled">
            	<option value="Y" <%if(vo.getEnabled().equals("Y")) out.print("selected");%>>启用&nbsp;&nbsp;&nbsp;&nbsp;</option>
            	<option value="N" <%if(vo.getEnabled().equals("N")) out.print("selected");%>>禁用&nbsp;&nbsp;&nbsp;&nbsp;</option>
            </td>
          </tr>
          <tr> 
            <td>部门排序</td>
